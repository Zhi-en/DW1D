...
import ntplib #ntp server (for time)
...
def ntptime():
    """Called to in get_time() to get current time from ntp server (due to RasPi not getting time.time correctly)"""
    c = ntplib.NTPClient()
    response = c.request('192.168.2.11', version=3) #Martin's RasPi
